...
def visualize_scene_view_samples(SCENE_DIR, SCENE_ALL_VIEWS_FILE, SCENE_ACCEPTED_VIEWS_FILE, CONFIG, verbose=True):
    ...
    with open(SCENE_ALL_VIEWS_FILE, 'r') as csvfile:
        ...
        for pose_meta in pose_reader:
            scene_name, view_idx, pos_idx, rot_idx, x_pos, y_pos, z_pos, roll, pitch, yaw, accepted = pose_meta
            ...
            view_idx_int, pos_idx_int, rot_idx_int = int(view_idx), int(pos_idx), int(rot_idx)
            x_pos, y_pos, z_pos = float(x_pos), float(y_pos), float(z_pos)
            roll, pitch, yaw = float(roll), float(pitch), float(yaw)

            if pos_idx_int not in visualized_pos_samples:

                # Record positions already added to visualization
                visualized_pos_samples.append(pos_idx_int)

                # Create sphere and add it to the scene at sampled position
                sphere_obj = get_sphere((x_pos, y_pos, z_pos), rot=(0,0,0), name="Initial-Sample-Position-ID-"+str(pos_idx), mat=sphere_mat)
    ...
    with open(SCENE_ACCEPTED_VIEWS_FILE, 'r') as csvfile:
        ...
        for pose_meta in pose_reader:
            ...
            # Parse pose infomration out of string type
            view_idx_int, valid_view_idx_int, pos_idx, rot_idx = int(view_idx), int(valid_view_idx), int(pos_idx), int(rot_idx)
            x_pos, y_pos, z_pos = float(x_pos), float(y_pos), float(z_pos)
            roll, pitch, yaw = float(roll), float(pitch), float(yaw)

            camera_sample = get_camera(pos=(x_pos, y_pos, z_pos), 
                rot=(pitch,yaw,roll), 
                name="Accepted-Sample-Valid-View-ID-"+str(valid_view_idx), 
                rot_mode='ZXY',
...
if __name__ == "__main__":
    ...
    scene_has_sampled_views = os.path.isfile(scene_all_view_poses_path) and os.path.isfile(scene_accepted_view_poses_path)

    if scene_has_semantic_mesh and scene_has_semantic_txt and scene_has_sampled_views:
        visualize_scene_view_samples(scene_dir_path, scene_all_view_poses_path, scene_accepted_view_poses_path, config, verbose=args.verbose)
