...
def make_cfg(settings):
    ...
    agent_cfg.action_space = {
        "move_forward": habitat_sim.agent.ActionSpec(
            "move_forward", habitat_sim.agent.ActuationSpec(amount=0.0001)
        ),
        "turn_left": habitat_sim.agent.ActionSpec(
            "turn_left", habitat_sim.agent.ActuationSpec(amount=1.50)
        ),
        "turn_right": habitat_sim.agent.ActionSpec(
            "turn_right", habitat_sim.agent.ActuationSpec(amount=1.50)
        ),
    }
...
def do_sim(sim_ix, sim_settings, scene_name, num_paths=1):
    ...
    for path_ix in range(num_paths):
        ...
        print(navmesh_settings.cell_size)
        print(navmesh_settings.cell_height)
        print(navmesh_settings.agent_height)
        print(navmesh_settings.agent_radius)
        print(navmesh_settings.verts_per_poly)
        raise
...
if __name__ == "__main__":
    ...
    parser.set_defaults(source_dataset="./zero_shot_scene_segmentation/datasets/raw_data/HM3D/",    
                        source_split="train",
                        dest_dir="./zero_shot_scene_segmentation/datasets/raw_data/samples/",
                        sim_height=1.65)
