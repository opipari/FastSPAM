...
aws s3 cp s3://vesta-intern-anthony/video_panoptic_segmentation/datasets/MVPd/MVPd/MVPd_train_detectron2_4.json ./MVPd/ > /dev/null
...
aws s3 cp s3://vesta-intern-anthony/video_panoptic_segmentation/models/LLVIS/models/swin_model_final_3.pth ./ > /dev/null
cd ..
echo "Downloaded pretrained models"

./scripts/train_4_swin.sh
