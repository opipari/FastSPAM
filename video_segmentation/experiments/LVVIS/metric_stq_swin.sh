...
cd results
aws s3 cp s3://vesta-intern-anthony/video_panoptic_segmentation/models/LLVIS/results/ov2seg_swin.tar.gz ./ > /dev/null
tar -xf ov2seg_swin.tar.gz
rm ov2seg_swin.tar.gz
cd ..
echo "Downloaded pretrained models"

python video_segmentation/metrics/evaluate_STQ.py --rle_path ./results/ov2seg_swin --ref_path ./video_segmentation/datasets/MVPd/MVPd --ref_split test --compute

rm -rf ./results/ov2seg_swin/panomasksRLE
