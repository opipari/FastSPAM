...
cd results
aws s3 cp s3://vesta-intern-anthony/video_panoptic_segmentation/models/fastsam_spam/evaluate_trained_fastsam_automatic.tar.gz ./ > /dev/null
tar -xf evaluate_trained_fastsam_automatic.tar.gz
rm evaluate_trained_fastsam_automatic.tar.gz
cd ..
echo "Downloaded pretrained models"

python video_segmentation/metrics/evaluate_STQ.py --rle_path ./results/evaluate_trained_fastsam_automatic --ref_path ./video_segmentation/datasets/MVPd/MVPd --ref_split test --compute

rm -rf ./results/evaluate_trained_fastsam_automatic/panomasksRLE
mv res.json results/
