...
cd results
aws s3 cp s3://vesta-intern-anthony/video_panoptic_segmentation/models/Tube-Link/results/evaluate_trained_tube_link_swin.tar.gz ./ > /dev/null
tar -xf evaluate_trained_tube_link_swin.tar.gz
rm evaluate_trained_tube_link_swin.tar.gz
cd ..
echo "Downloaded pretrained models"

python video_segmentation/metrics/evaluate_STQ.py --rle_path ./results/evaluate_trained_tube_link_swin --ref_path ./video_segmentation/datasets/MVPd/MVPd --ref_split test --compute

rm -rf ./results/evaluate_trained_tube_link_swin/panomasksRLE
mv res.json results/
