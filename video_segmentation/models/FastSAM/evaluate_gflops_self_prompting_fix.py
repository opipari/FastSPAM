...
class FastSelfPrompting(nn.Module):
    ...
    def reprojected_memory_coords(self, image_shape, depth, camera):
    ...
    def point_prompt(self, points, pointlabel):  # numpy 
        if self.everything_results == None:
            return []
        masks = self.everything_results # NxHxW
        target_height = 480
        target_width = 640
        h = masks.shape[1]
        w = masks.shape[2]
        ...
        onemask = torch.zeros((h, w))
        masks = masks[torch.argsort(masks.sum(dim=(1,2)), descending=True)]
        for i, mask in enumerate(masks):
            for i, point in enumerate(points):
                ...
                #    onemask[mask] = 0
        onemask = onemask >= 1
        return onemask.unsqueeze(0)


    def forward(self, image_shape, depth, xy_depth, camera_info, everything_results):
        self.everything_results = everything_results
        camera = get_cameras(*camera_info).to('cuda')
        xyz = camera.unproject_points(xy_depth, from_ndc=True, world_coordinates=True)
        
        points_ = self.reprojected_memory_coords(image_shape, depth, camera)
        if points_ is None:
            ann = self.everything_results
            if len(ann)>0:
                ann = ann.to(torch.bool)
                ann = ann[torch.argsort(torch.sum(ann,(1,2)),descending=True)]
        else:
            ann = []
            all_ann = self.everything_results
            if len(all_ann)>0:
                points = points_[:1]
                ann = self.point_prompt(points=points, pointlabel=[1])
                for p in points_[1:]:
                    mask = self.point_prompt(points=[p], pointlabel=[1])
    ...
    with torch.no_grad():
        for vi, video in enumerate(tqdm(dataset, position=0, disable=index!=0)):
            ...
            for idx, sample in enumerate(tqdm(video, position=1, disable=index!=0)):
                ...
                camera_info = (torch.as_tensor(sample['camera']['K']),
                                    torch.as_tensor(sample['camera']['W2V_pose']),
                                    torch.as_tensor(sample['meta']['image_size']))#.to('cuda')

                input = Image.fromarray(image)
                input = input.convert("RGB")
                everything_results = model.model(
                    input,
                    device=0,
                    retina_masks=model.config["retina"],
                    imgsz=model.config["imgsz"],
                    conf=model.config["conf"],
                    iou=model.config["iou"]
                    )
                everything_results = torch.stack([everything_results[0].masks.data[i] == 1.0 for i in range(len(everything_results[0].masks.data))])
